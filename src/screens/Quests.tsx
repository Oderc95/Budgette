    ...
    <div className="flex flex-col gap-5">
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-4 px-5 py-4">
          <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-sage-soft text-sage-deep">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {multiplier > 1 && (
              <Chip tone="plum" icon="TrendingUp">
                XP ×{multiplier}
              </Chip>
            )}
          </div>
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                active ? 'border-sage bg-sage-soft text-sage-deep' : 'border-line bg-surface text-ink-soft hover:bg-surface-2',
              )}
            >
              <Icon name={CADENCE_META[item].icon} size={15} />
              {CADENCE_META[item].label}
              <span className={clsx('tabular rounded-full px-1.5 text-[0.72rem]', active ? 'bg-sage text-[#fffcf7]' : 'bg-surface-2')}>
        ...
        <div className="grid gap-3 px-5 pb-5 lg:grid-cols-2">
          {list.map((challenge) => (
            <div key={challenge.id} className="relative">
              {featured.has(challenge.id) && (
                <span className={clsx('chip absolute -top-2 left-4 z-10 shadow-soft', TONE.gold.bg, TONE.gold.text)}>
      ...
      <Card>
        <CardHeader title="Comment ça compte" icon="Info" tone="sky" />
